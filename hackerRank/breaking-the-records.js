...
function getRecord(s){
    // this isn't super intuitive, but I thought it would be a cute solution to an otherwise bland problem:
    // the high scores are all logged, and instead of having a highScoreChanges variable to return we can
    // just return the length of all the changes stored in an array

    let highScore = [s[0]];
    let lowScore = [s[0]];

    s.forEach(function(score) {
        if (score > highScore[highScore.length-1]) highScore.push(score);
        if (score < lowScore[lowScore.length-1]) lowScore.push(score);
    })

    return [highScore.length-1, lowScore.length-1];
}
